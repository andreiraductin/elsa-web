...
<p>This plugin handles two-way conversion between <a href="http://geojson.org/geojson-spec.html">GeoJSON</a> and the <a href="http://resources.arcgis.com/en/help/arcgis-rest-api/index.html#/Geometry_objects/02r3000000n1000000/">ArcGIS Geometry</a> format used by Esri.</p>
<h2>
...
<span class="kd">var</span> <span class="nx">arcgisPoint</span> <span class="o">=</span> <span class="nx">Terraformer</span><span class="p">.</span><span class="nx">ArcGIS</span><span class="p">.</span><span class="nx">convert</span><span class="p">({</span>
...
<td>By default, when converting GeoJSON Features the <code>id</code> key of your output feature will be set on the <code>OBJECTID</code> field. This option allows you to assign your ID using a different fieldname.</td>
</tr>
</tbody></table>
<h5>Notes</h5>
<p>Terraformer will also handle converting <code>FeatureCollection</code> and <code>GeometryCollection</code> objects to arrays of ArcGIS geometries or features. However it will <strong>not</strong> do this in reverse. This is because there is no official structure for arrays of features and geometries in ArcGIS and output features will be missing an <code>id</code> property. See <a href="https://github.com/Esri/Terraformer/issues/104">this issue</a> for more details.</p>
...
<td>This is used to set the value of <code>spatialReference.wkid</code> in the output. Setting <code>sr</code> <strong>does not</strong> convert the <code>geojson</code> coordinates, it only identifies the projection using the appropriate code.</td>
</tr>
<tr>
<td><code>idAttribute</code></td>
<td><code>String</code></td>
...
<h5>Example</h5><pre class="highlight javascript"><code><span class="c1">// take a Terraformer.Primitive or GeoJSON and convert it to an ArcGIS JSON object</span>
